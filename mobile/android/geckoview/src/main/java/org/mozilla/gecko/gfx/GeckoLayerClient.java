...
class GeckoLayerClient implements LayerView.Listener, PanZoomTarget
{
    ...
    private void adjustViewport(MotionEvent ev) {
        String json = "";
        try {
            JSONObject jsonObj = new JSONObject();
            jsonObj.put("zoom", mViewportMetrics.zoomFactor);
            jsonObj.put("x", mViewportMetrics.viewportRectLeft);
            jsonObj.put("y", mViewportMetrics.viewportRectTop);
            jsonObj.put("w", mViewportMetrics.viewportRectWidth);
            jsonObj.put("h", mViewportMetrics.viewportRectHeight);
            jsonObj.put("touchCount", ev.getPointerCount());

            JSONArray xCoords = new JSONArray();
            JSONArray yCoords = new JSONArray();
            for (int i=0; i<ev.getPointerCount(); i++) {
                xCoords.put(ev.getX(i)/mViewportMetrics.zoomFactor);
                yCoords.put(ev.getY(i)/mViewportMetrics.zoomFactor);
            }
            jsonObj.put("xCoords", xCoords);
            jsonObj.put("yCoords", yCoords);

            json = jsonObj.toString();
        } catch (Exception e) {
            Log.e(LOGTAG, "Unable to convert point to JSON", e);
        }
        GeckoAppShell.notifyObservers("Viewport:Change", json);
    }
    ...
    public void updateRootFrameMetrics(float scrollX, float scrollY, float zoom) {
        ...
        if (mLastMotionEvent != null) {
            adjustViewport(mLastMotionEvent);
        }
    }
    ...
    @Override
    public void setAnimationTarget(ImmutableViewportMetrics metrics, MotionEvent ev) {
        if (mGeckoIsReady) {
