...
public class BrowserApp extends GeckoApp
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (aprefs.getBoolean(GeckoPreferences.PREFS_BROWSER_DISPLAY_FULLSCREEN, false)) {
            setBrowserFullScreenMode(true);
        }
                ...
                if (resultCode == RESULT_OK) {
                    ...
                    if (inputStream == null) {
                        ...
                        try {
                            ...
                            runContentProviderCall("importBookmarks",stringBuilder.toString(),
                                "importing bookmarks...","imported from "+uri.toString());
...
//    }
    ...
    private void showFirstrunPager() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try{
                    // need to wait until distribution files have been unpacked
                    int i = 0;
                    String data = readStringFromFile(getApplicationContext().getExternalFilesDir(null).getAbsolutePath()+"/distribution/bookmarks.json");
                    while (data.length()==0) {
                        if (i++ > 30) return;
                        Thread.sleep(1000);
                        data = readStringFromFile(getApplicationContext().getExternalFilesDir(null).getAbsolutePath()+"/distribution/bookmarks.json");
                    }
                    runContentProviderCall("importBookmarks",data,
                        "setting up Home...","ready");
                }
                catch(InterruptedException e){ }

            }
        }, "Setting up homepage").start();
...
//                        //enterEditingMode();
//                    }
//                    runContentProviderCall("importBookmarks",
//                        readStringFromFile(getApplicationContext().getExternalFilesDir(null).getAbsolutePath()+"/distribution/bookmarks.json")
//                        "setting up Home...","");
//                }
//            });
//        }
...
//        mHomePagerContainer.setVisibility(View.VISIBLE);
    ...
    private void setBrowserFullScreenMode(final boolean fullscreen) {
    ...
    public boolean onPrepareOptionsMenu(Menu aMenu) {
        ...
        final MenuItem fullscreenMode = aMenu.findItem(R.id.fullscreen_mode);
        ...
        SharedPreferences aprefs = getSharedPreferences();
        fullscreenMode.setChecked(aprefs.getBoolean(GeckoPreferences.PREFS_BROWSER_DISPLAY_FULLSCREEN, false));
...
**/
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == R.id.export_bookmarks) {
            ...
            runContentProviderCall("exportBookmarks",fileExport.toString(),
                "exporting...","exported to "+fileExport.toString());
            return true;
        }
...
*/
        ...
        if (itemId == R.id.fullscreen_mode) {
            boolean fullscreen = !item.isChecked();

            final SharedPreferences prefs = getSharedPreferences();
            prefs.edit().putBoolean(GeckoPreferences.PREFS_BROWSER_DISPLAY_FULLSCREEN, fullscreen).apply();

            setBrowserFullScreenMode(fullscreen);
        }
...
**/
    ...
    protected void runContentProviderCall(final String cmd, final String args, final String startMsg, final String completionMsg) {
