...
//    }
    ...
    private String readStringFromApk(String packagePath, String pathSuffix) throws IOException {
    ...
    private void importDefaultBookmarks() {
        try {
            String data = readStringFromApk(getApplicationContext().getPackageResourcePath(),"bookmarks.json");
            runContentProviderCall("importBookmarks",data,"setting up Home...","ready");            
        } catch (java.io.IOException e) {
            Log.e(LOGTAG, "Error reading stream ", e);
        }
    }
...
**/
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == R.id.import_default_bookmarks) {
            importDefaultBookmarks();
            return true;
        }
    ...
    protected void runContentProviderCall(final String cmd, final String args, final String startMsg, final String completionMsg) {
        if (startMsg.length()>0) {
            SnackbarBuilder.builder(BrowserApp.this)
                .message(startMsg)
                .duration(completionMsg.length() > 0?Snackbar.LENGTH_INDEFINITE:Snackbar.LENGTH_LONG)
                .buildAndShow();
        }
