...
// pyllq +++
// this is where width changes is propagated to Gecko
function _changeMaxLineBoxWidth(aWidth) {
  ...
  try {
    ...
    console.log("changeMaxLineBoxWidth:"+aWidth);
...
var gFitImages = true;
var gHideOverlays = true;
var gPaddedBody=false;
var gViewport = null;
var gSnapToRange = null;
var gSnapToRangeSpan = null;
var gClampImagesTab = null;
Strings.init();

function _updateViewportMetadata(aViewport) {
  aViewport.z = aViewport.baseZoom / aViewport.zoom;
  aViewport.baseWidth = aViewport.w / aViewport.baseZoom;
  aViewport.width = aViewport.w / aViewport.zoom;
  aViewport.reflowWidth = (aViewport.zoom <= aViewport.baseZoom)?
                        (aViewport.w / aViewport.baseZoom):
                        (aViewport.w / aViewport.zoom);  
}

function _rotateViewport(aViewport) {
  console.log("screen rotated");
  var w = aViewport.w;
  aViewport.w = aViewport.h; aViewport.h = w;

  aViewport.yCoords = [12]; // use a top line for rotation snapping

  _updateViewportMetadata(aViewport);
}

var snapCount=0;
function _snapRange(aViewport) {
  ...
  // sample across the top half of the page until we get a textnode to lock on
  var nSteps = [1.0,0.8,1.2,0.6,1.4,0.4,1.6,0.2,1.8];
  //var zHeight = aViewport.h/aViewport.zoom;
  aViewport.yCoords.sort(function(a, b){return a-b});
  var zHeight = aViewport.yCoords[0];
  var fac;
  while (fac = nSteps.shift()) {
    var range = document.caretPositionFromPoint((aViewport.w/2)/aViewport.zoom, zHeight*fac);
    if (range && range.offsetNode && range.offsetNode.nodeType == 3 && range.offset) {
      gSnapToRange = range.offsetNode;
      var replacement = gSnapToRange.splitText(range.offset);
      var span = document.createElement('span');
      gSnapToRange.parentNode.insertBefore(span, replacement);
      //span.textContent = "XX" + snapCount +"XX"; snapCount++;
      gSnapToRange = span;

      // this is effectively delayed removal to avoid a possible race condition
      //if (gSnapToRangeSpan)
      //  gSnapToRangeSpan.parentNode.removeChild(gSnapToRangeSpan);
      gSnapToRangeSpan = span;
...
function _snapIntoView() {
  if (gSnapToRange) {
    if (gSnapToRange.nodeType==1) {

      console.log("snap locA:"+BrowserApp.selectedTab.window.document.documentElement.scrollTop);
      gSnapToRange.scrollIntoView();
      console.log("snap locB:"+BrowserApp.selectedTab.window.document.documentElement.scrollTop);
      BrowserApp.selectedTab.window.document.documentElement.scrollTop -= 64;
      console.log("snap locC:"+BrowserApp.selectedTab.window.document.documentElement.scrollTop);

      if (gSnapToRange.parentElement.getBoundingClientRect().left == 0) {
        gPaddedBody = true;
        var w = gViewport.reflowWidth*0.025;
        BrowserApp.selectedTab.window.document.body.style.paddingLeft = w+"px"; // 2.5%
      }
    }
    gSnapToRange = null;
  }
...
var BrowserApp = {
  ...
  startup: function startup() {
    ...
    Services.obs.addObserver(this, "Viewport:Change");
    Services.obs.addObserver(this, "Viewport:Flush");
...
//                       (aTarget.videoWidth != 0 && aTarget.videoHeight != 0));
  ...
  observe: function(aSubject, aTopic, aData) {
    console.log("BrowserApp:observe:"+aTopic);
    let browser = this.selectedBrowser;
...
var FixerUserAgent = {
  ...
  UA_FF: "Mozilla/5.0 (Android 4.2.2; Mobile; rv:50.0) Gecko/50.0 Firefox/50.0",
  UA_PYLLQ:  "Mozilla/5.0 (Linux; U; Android 4.2.2; en;) AppleWebkit/537.36 (KHTML, like Gecko) Version/4.0 Mobile Safari/537.36 Pyllq/50.0",
  UA_CHROME: "Mozilla/5.0 (Linux; Android 4.2.2; 2013023 Build/HM2013023) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.91 Mobile Safari/537.36",

  init: function ua_init() {
    try {
      Services.obs.addObserver(this.HTTP_on_useragent_request.bind(this), "http-on-useragent-request", false);
    } catch (x) {
    }

    this.DEFAULT_UA = Cc["@mozilla.org/network/protocol;1?name=http"]
  ...
  HTTP_on_useragent_request: function(aSubject, aTopic, aData) {
    let channel = aSubject.QueryInterface(Ci.nsIHttpChannel);

    // fix for mozilla addon
    if (channel.URI.host.match(/mozilla\.org/)) {
      channel.setRequestHeader("User-Agent", this.UA_FF, false);
      return;
    }

    if (channel.URI.host.match(/weibo\.(com|cn)/)) {
      channel.setRequestHeader("User-Agent", this.UA_CHROME, false);
      return;
    }

    //channel.setRequestHeader("User-Agent", this.UA_PYLLQ, false);
  }
};
...
// pyllq +++
  _clampImages: function(aViewport) {
    if (!gFitImages) return;

    var tab = BrowserApp.selectedTab;
    var document = tab.window.document;
    ...
    if (gFitImages) {
      // vw is relative to frames
      var strMaxWidth = Math.round(aViewport.width) + "px";
      console.log("gFitImages maxWidth="+strMaxWidth);

      var a = document.getElementsByTagName("img");
      for (var i=0; i < a.length; i++) {
        var e = a[i];
        if (!e._origAlign) e._origAlign = e.align;
        if (!e._origMarginRight) e._origMarginRight = e.style.marginRight;
        e.style.maxWidth = strMaxWidth;
        var rect = e.getBoundingClientRect();
        //console.log("e.clientWidth:"+e.clientWidth);
        //console.log("aViewport.width:"+aViewport.width);
        //e.style.marginRight = (e.clientWidth < aViewport.width)?"":
        //          Math.round(aViewport.baseWidth - aViewport.width) + "px";
        e.style.marginRight = (rect.right <= aViewport.width)?"":
                  Math.round(aViewport.baseWidth - aViewport.width) + "px";
        //console.log("e.style.marginRight:"+e.style.marginRight);
        //e.align = (z==1)?e._origAlign:"left";
      }
    }

    if (z==1 && gPaddedBody) {
      gPaddedBody = false;
      BrowserApp.selectedTab.window.document.body.style.paddingLeft = "";      
    }
  },

  _hideOverlays: function(aViewport) {
    if (!gHideOverlays) return;
    ...
    if (gHideOverlays) {
      ...
      for (let e of fixedNodes) {
        if (e.style._origDisplay==null) {
          e.style._origDisplay = e.style.display;
          e.style._origOpacity = e.style.opacity;
        }
        e.style.display = (z==1)?e.style._origDisplay:"none";
        e.style.opacity = (z==1)?e.style._origOpacity:0;
      }

    }
  },

  _reflowOnZoom: function(aViewport) {
    _changeMaxLineBoxWidth(aViewport.reflowWidth*0.95);
  },
// pyllq ---
...
/*
  {"zoom":4.313073635101318,"x":8.29376220703125,"y":1560.18896484375,"w":1080,"h":1860,
  "touchCount":2,"xCoords":[146.6273193359375,38.68367004394531],"yCoords":[168.92575073242188,312.136474609375],
  "baseZoom":3,"z":0.695559652769417,
  "baseWidth":360,"width":250.40147499699012,"reflowWidth":250.40147499699012}
*/
  onViewportChange: function(aViewport) {

    aViewport.baseZoom = aViewport.w / window.innerWidth;
    _updateViewportMetadata(aViewport);

    console.log("onViewportChange:"+JSON.stringify(aViewport));

    if (aViewport.touchCount > 3) return;

    // for multitouch, we only hideOverlays and exit
    if (aViewport.touchCount == 3) {
      this._hideOverlays(aViewport); return;
    }

    if (gViewport && gViewport.zoom == aViewport.zoom
      && gViewport.w == aViewport.w) return;
    gViewport = aViewport;
    gViewport.tab = this;

    _snapRange(aViewport);
    this._reflowOnZoom(aViewport);

    gClampImagesTab = this;
  },
...
var BrowserEventHandler = {
  init: function init() {
    ...
    Services.prefs.addObserver("browser.zoom.pyllq-fit-images", this, false);
    Services.prefs.addObserver("browser.zoom.pyllq-hide-overlays", this, false);
...
// pyllq +++
  updateReflozPref: function() {
    ...
    gFitImages = Services.prefs.getBoolPref("browser.zoom.pyllq-fit-images");
    gHideOverlays = Services.prefs.getBoolPref("browser.zoom.pyllq-hide-overlays");
  },
// pyllq ---

  handleEvent: function(aEvent) {
    console.log("handleEvent:"+aEvent.type);
    switch (aEvent.type) {
      case 'touchstart':
      ...
      case 'touchend':
        ...
        _snapIntoView();
        if (gClampImagesTab) {
          gClampImagesTab._clampImages(gViewport);
          gClampImagesTab._hideOverlays(gViewport);
          gClampImagesTab = null;
        }
        // pyllq ---
        if (this._inCluster) {
          aEvent.preventDefault();
    ...
    if (aTopic == "nsPref:changed") {
      switch (aData) {
        case "browser.zoom.reflowOnZoom":
          this.mReflozPref = Services.prefs.getBoolPref("browser.zoom.reflowOnZoom");
          break;
        case "browser.zoom.pyllq-fit-images":
          gFitImages = Services.prefs.getBoolPref("browser.zoom.pyllq-fit-images");
          break;
        case "browser.zoom.pyllq-hide-overlays":
          gHideOverlays = Services.prefs.getBoolPref("browser.zoom.pyllq-hide-overlays");
          break;
      }      
      return;
  ...
  observe: function(aSubject, aTopic, aData) {
    ...
    switch (aTopic) {
      case "Window:Resize":
        ...
        // includes screen orientation change
        gScreenWidth = window.outerWidth * window.devicePixelRatio;
        gScreenHeight = window.outerHeight * window.devicePixelRatio;
        let tabs = BrowserApp.tabs;

        // perform reflow after rotation
        if (gViewport && gViewport.tab) {
          if ((gScreenWidth > gScreenHeight && gViewport.w < gViewport.h) ||
              (gScreenWidth < gScreenHeight && gViewport.w > gViewport.h)) {
            _rotateViewport(gViewport);
            _snapRange(gViewport);
            gViewport.tab._reflowOnZoom(gViewport);

            gViewport.tab._clampImages(gViewport);
            _snapIntoView();
          }
        }
