...
gfxFT2FontList::FindFonts()
{
    ...
    FindFontsInDir(sysfonts, mFontNameCache.get(), FT2FontFamily::kVisible);
    ...
    FindFontsInDir(sdcard, mFontNameCache.get(), FT2FontFamily::kVisible);

    nsCString distribution;
    char *envdistribution = PR_GetEnv("PYXDB_EXT_DIR");
    if (envdistribution) {
        distribution = envdistribution;
        FindFontsInDir(distribution, mFontNameCache.get(), FT2FontFamily::kVisible);
    }
                               ...
                               const char* sFaultyFonts[])
{
    ...
    ALOG("loadfont: gfxFT2FontList::FindFontsInDir: %s",aDir.get());
    ...
    while ((ent = readdir(d)) != nullptr) {
        ...
        //ALOG("loadfont:checking for faulty font: %s",ent->d_name);
        if (sFaultyFonts && isInList(sFaultyFonts,ent->d_name)) {
            ALOG("loadfont:faulty font: %s",ent->d_name);
            continue;
        }        
