                   ...
                   nsTextFrameUtils::Flags aFlags2)
{
    ...
    aFlags &= ~(gfx::ShapedTextFlags::TEXT_ENABLE_SPACING);
...
//        uint32_t glyphIndex = cg->GetSimpleGlyph();

        if (!cg->IsSimpleGlyph() && cg->GetGlyphCount()) {
            ++cg; continue;
        }

//        if (cg->IsSimpleGlyph() && canBreak==0)
//            canBreak = CompressedGlyph::FLAG_BREAK_TYPE_NORMAL;
